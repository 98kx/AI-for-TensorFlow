...
xs = tf.compat.v1.placeholder(tf.float32, [None, 32, 32, 3])  #每张图片32*32*3个点
ys = tf.compat.v1.placeholder(tf.int32, [None])               #每个样本有1个输出
# 存放DropOut参数的容器 
drop = tf.compat.v1.placeholder(tf.float32)                   #训练时为0.25 测试时为0

# 定义卷积层 conv0
conv0 = tf.compat.v1.layers.conv2d(xs, 20, 5, activation=tf.nn.relu)    #20个卷积核 卷积核大小为5 Relu激活
# 定义max-pooling层 pool0
pool0 = tf.compat.v1.layers.max_pooling2d(conv0, [2, 2], [2, 2])        #pooling窗口为2x2 步长为2x2
print("Layer0：\n", conv0, pool0)
 
# 定义卷积层 conv1
conv1 = tf.compat.v1.layers.conv2d(pool0, 40, 4, activation=tf.nn.relu) #40个卷积核 卷积核大小为4 Relu激活
# 定义max-pooling层 pool1
pool1 = tf.compat.v1.layers.max_pooling2d(conv1, [2, 2], [2, 2])        #pooling窗口为2x2 步长为2x2
print("Layer1：\n", conv1, pool1)

# 将3维特征转换为1维向量
flatten = tf.compat.v1.layers.flatten(pool1)
...
# 加上DropOut防止过拟合
dropout_fc = tf.compat.v1.layers.dropout(fc, drop)
...
losses = tf.nn.softmax_cross_entropy_with_logits(
        labels = tf.stop_gradient( tf.one_hot(ys, num_classes)),       #将input转化为one-hot类型数据输出
        logits = logits)

# 平均损失
mean_loss = tf.reduce_mean(input_tensor=losses)

# 定义优化器 学习效率设置为0.0001
optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=1e-4).minimize(losses)
